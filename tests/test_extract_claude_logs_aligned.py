...
class TestClaudeConversationExtractor(unittest.TestCase):
    ...
    def test_init_with_none_output_fallback(self):
        ...
        # Make touch() fail so the writable-directory check fails for all candidates,
        # but allow mkdir itself to succeed so the final fallback path works.
        with patch("pathlib.Path.touch", side_effect=Exception("Permission denied")):
    ...
    def test_find_sessions_with_project_filter(self):
        ...
        with patch("pathlib.Path.home", return_value=Path(self.temp_dir)):
            # Create extractor inside the patch so claude_dir uses patched home
            test_extractor = ClaudeConversationExtractor(self.temp_dir)
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    print_calls = [str(call) for call in mock_print.call_args_list]
                    self.assertTrue(any("건너뜀" in str(call) for call in print_calls))
    ...
    def test_extract_multiple_invalid_indices(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            print_calls = [str(call) for call in mock_print.call_args_list]
            self.assertTrue(
                any("잘못된 세션 번호" in str(call) for call in print_calls)
            )
...
class TestMainFunction(unittest.TestCase):
    ...
    def test_main_no_args_lists_sessions(self):
        """Test that no arguments defaults to listing sessions"""
    ...
    def test_main_search(self):
        ...
        with patch("sys.argv", ["extract_claude_logs.py", "--search", "test query"]):
            ...
            with patch.dict("sys.modules", {"search_conversations": Mock(ConversationSearcher=Mock(return_value=mock_searcher))}):
                with patch("builtins.print"):
