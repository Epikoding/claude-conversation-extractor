...
class TestClaudeConversationExtractorComprehensive(unittest.TestCase):
    ...
    def test_find_sessions_empty(self):
        ...
        # Create a temp dir that has no .claude/projects with jsonl files
        empty_dir = Path(self.temp_dir) / "empty_home"
        empty_claude = empty_dir / ".claude" / "projects"
        empty_claude.mkdir(parents=True)

        extractor = ClaudeConversationExtractor(self.temp_dir)
        # Override claude_dir to point to the empty location
        extractor.claude_dir = empty_claude
    ...
    def test_find_sessions_with_files(self):
        ...
        # Override claude_dir to point to our test location
    ...
    def test_extract_text_content_empty(self):
        ...
        self.assertEqual(self.extractor._extract_text_content(""), "")
        # None now falls through to str(content) which returns "None"
    ...
    def test_list_recent_sessions(self):
        """Test listing sessions with details (method renamed to list_recent_sessions)"""
        # Create test file
        project_dir = self.claude_dir / "test_project"
        project_dir.mkdir()
    ...
    def test_extract_recent_via_main(self):
        """Test --recent flag via main function"""
        with patch("sys.argv", ["extract_claude_logs.py", "--recent", "5"]):
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch("builtins.print"):
                        main()
                        # --recent N now calls extract_multiple with indices [0..N-1]
                        mock_extract.assert_called_once()
                        args = mock_extract.call_args[0]
                        self.assertEqual(args[1], [0, 1, 2, 3, 4])

    def test_extract_all_via_main(self):
        """Test --all flag via main function"""
        mock_sessions = [Path(f"session{i}.jsonl") for i in range(5)]
        ...
        with patch("sys.argv", ["extract_claude_logs.py", "--all"]):
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch("builtins.print"):
                        main()
                        # --all calls extract_multiple with all indices
                        mock_extract.assert_called_once()
                        args = mock_extract.call_args[0]
    ...
    def test_main_search(self):
        ...
        with patch("sys.argv", ["extract_claude_logs.py", "--search", "test query"]):
            with patch("search_conversations.ConversationSearcher") as mock_searcher_cls:
